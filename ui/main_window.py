...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        # 添加各个选项卡
        self.tabs.addTab(self._create_process_tab(), "📄 处理中心")
        ...
        self.tabs.addTab(self._create_settings_tab(), "⚙️ 系统设置")
    ...
    def _init_menu_bar(self):
        ...
        setup_action = QAction("重新运行快速上手向导", self)
    ...
    def _check_first_run(self):
        ...
        # 检测条件：config.json 不存在 或 监控目录为空且无非 always 规则
    ...
    def _run_setup_wizard(self):
        ...
        if wizard.exec_() == QWizard.Accepted:
            ...
            self.log(f"向导完成 — 模式: {result['mode_name']} | "
            ...
            if result['monitor_dirs']:
                self.config_mgr.set('monitor_dirs', result['monitor_dirs'])
                self.config_mgr.save()
                self.monitor_panel.refresh_dirs()

            # 应用模式预置规则
            mode_key = result['mode']
            mode_info = wizard.PRESET_MODES.get(mode_key, {})
            if mode_info.get('default_rules'):
                existing_rules = self.config_mgr.config.setdefault('rules', [])
                # 仅当没有自定义规则时才导入
                has_custom = any(
                    r.get('condition_type') != 'always' or len(r.get('steps', [])) > 0
                ...
                if not has_custom:
                    self.config_mgr.config['rules'] = mode_info['default_rules']
                    self.config_mgr.save()
                    self._load_rules()
    ...
    def _show_help(self):
        """显示使用帮助（使用可搜索的帮助浏览器）"""
        HelpBrowser.instance(self).show_topic("overview")
    ...
    def _create_process_tab(self) -> QWidget:
        ...
        self.drop_zone.files_added.connect(self._on_drop_zone_files_added)
        self.drop_zone.process_requested.connect(self._on_drop_zone_process)
        self.file_list = self.drop_zone.file_list  # 保持外部引用兼容
        self.file_list.setMinimumHeight(120)
        self.file_list.setMaximumHeight(200)
        self.file_list.setContextMenuPolicy(Qt.CustomContextMenu)
        self.file_list.customContextMenuRequested.connect(self._show_file_context_menu)
        # 文件计数标签（引用 DropProcessingZone 的内置标签）
        ...
        default_output = self.config_mgr.get('output_dir', str(Path.home() / "Desktop" / "定稿文件"))
        ...
        rule_buttons = [
            ("➕ 添加规则", self.add_rule, "添加新的处理规则"),
            ("✏️ 编辑规则", self.edit_rule, "编辑选中的规则"),
            ("🗑️ 删除规则", self.del_rule, "删除选中的规则"),
            ("⬆️ 上移", self.move_up, "将选中规则上移（提高优先级）"),
            ("⬇️ 下移", self.move_down, "将选中规则下移（降低优先级）"),
            ("📋 复制规则", self._duplicate_rule, "复制选中的规则"),
            ("🧩 可视化编辑", self._open_visual_rule_editor, "使用节点式可视化编辑器"),
        ]
    ...
    def _create_settings_tab(self) -> QWidget:
        ...
        self.qhi_edit.setText(self.config_mgr.get('qhi_path', QI_EXE))
        ...
        qhi_layout.addWidget(QPushButton("浏览...", clicked=self.browse_qhi))
        qhi_layout.addWidget(QPushButton("自动检测", clicked=self._auto_detect_qhi))
        ...
        self.rename_enabled.setChecked(self.config_mgr.get('rename_enabled', True))
        rename_layout.addRow("", self.rename_enabled)

        self.rename_template = QLineEdit()
        self.rename_template.setText(
            self.config_mgr.get('rename_template', '{seq}-{paper}-{pages}P-{name}')
        )
        self.rename_template.setPlaceholderText("支持变量: {seq}序号 {paper}纸张 {pages}页数 {name}原名 {binding}装订 {copies}份数")
        ...
        self.number_digits.setValue(self.config_mgr.get('number_digits', 3))
        ...
        self.number_start.setValue(self.config_mgr.get('number_start', 1))
        ...
        backup_btn.setToolTip("创建数据库备份文件")
        db_layout.addWidget(backup_btn)

        restore_btn = QPushButton("📥 恢复数据库")
        restore_btn.clicked.connect(self._restore_db)
        restore_btn.setToolTip("从备份文件恢复数据库")
        db_layout.addWidget(restore_btn)

        clear_orders_btn = QPushButton("🗑️ 清空订单")
        clear_orders_btn.clicked.connect(self.clear_orders)
        clear_orders_btn.setToolTip("删除所有订单记录")
        db_layout.addWidget(clear_orders_btn)

        reset_db_btn = QPushButton("⚠️ 重置数据库")
        reset_db_btn.clicked.connect(self.reset_db)
        reset_db_btn.setToolTip("删除并重建数据库（将丢失所有数据）")
        reset_db_btn.setStyleSheet("QPushButton { color: red; font-weight: bold; }")
    ...
    def _show_rule_context_menu(self, pos):
        ...
        menu.addAction("✏️ 编辑规则", self.edit_rule)
        menu.addAction("🗑️ 删除规则", self.del_rule)
        menu.addSeparator()
        menu.addAction("⬆️ 上移", self.move_up)
        menu.addAction("⬇️ 下移", self.move_down)
        menu.addSeparator()
