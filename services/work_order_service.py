...
class WorkOrderService:
    ...
    def generate_receipt(self, order: WorkOrder) -> Receipt:
        ...
        receipt = Receipt(
            ...
            items=[{"name": item.name, "qty": item.quantity, "price": item.unit_price, "subtotal": item.subtotal} for item in order.items],
...
class ReceiptPrinter:
    ...
    def format_receipt(receipt: Receipt) -> str:
        ...
        for item in receipt.items:
            ...
            lines.append(f"{name:<15}{qty:>5}{price:>8.2f}{subtotal:>8.2f}")
        
        lines.append("-" * 40)
        lines.append(f"{'Total:':<25}{'CNY ' + f'{receipt.total_amount:.2f}':>12}")
        
        if receipt.payment_amount > 0:
            lines.append(f"{'Paid:':<25}{'CNY ' + f'{receipt.payment_amount:.2f}':>12}")
            lines.append(f"{'Change:':<25}{'CNY ' + f'{receipt.change:.2f}':>12}")
